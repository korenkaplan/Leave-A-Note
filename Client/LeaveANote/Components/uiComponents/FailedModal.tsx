...
import animationData from '../../assets/lottie.Animation/BouncyFail.json';
...
const FailedModal: React.FC<Props> = ({ title, footerTitle, body, onSwipe, isVisible }) => {
...
const createStyles = (primary: string, secondary: string, text: IText, background: string, buttonMain: StyleButton, buttonAlt: StyleButton) =>
  StyleSheet.create({
    modalStylel: { borderWidth: 2, borderColor: primary, backgroundColor: background, minWidth: 250 },
    ...
    bodyText: {
      fontSize: 14, textAlign: 'center', color: text.secondary, marginTop: 10, maxWidth: 220
    },
    bodyView: {
    ...
    modalContent: {
      alignItems: 'center', paddingTop: 0, justifyContent: 'center'
    }
  });
